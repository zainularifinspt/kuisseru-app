          ...
          <div className="bg-[rgba(255,255,255,0.7)] backdrop-blur-xl border-2 border-deep-obsidian rounded-xl overflow-hidden relative shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
            ...
            <div className="p-8 sm:p-10">
              <form onSubmit={handleAuth} className="space-y-6">
                {isRegistering && (
                  <div className="space-y-2">
                    <label className="font-heading font-semibold text-sm text-on-surface-variant px-2 uppercase tracking-wide">Nama Lengkap</label>
                    <input 
                      type="text"
                      value={name}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center px-2">
                    <label className="font-heading font-semibold text-sm text-on-surface-variant uppercase tracking-wide">Kata Sandi</label>
                    {!isRegistering && (
                      <Link href="/teacher/forgot-password" className="text-electric-blue text-sm font-semibold hover:underline">
                        Lupa Password?
                      </Link>
                    )}
                  </div>
                ...
                >
                  ...
                  <div className="relative bg-transparent px-8 py-4 rounded-full flex items-center justify-center gap-2">
                    <span className="font-heading text-xl text-on-primary font-bold tracking-wide">
                      {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : isRegistering ? "Daftar Sekarang" : "Masuk"}
                    </span>
                  </div>
                </button>
              </form>
              ...
              <div className="relative my-8">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-[rgba(255,255,255,0.7)] text-on-surface-variant font-heading font-semibold">Atau</span>
                </div>
              </div>
              ...
              <button 
                ...
                disabled={isLoading}
                className="w-full bg-white text-deep-obsidian font-heading font-bold py-4 rounded-full flex items-center justify-center gap-3 border-2 border-deep-obsidian hover:bg-surface-variant hover:shadow-[4px_4px_0px_rgba(10,10,10,1)] transition-all cursor-pointer"
              >
              ...
              <div className="mt-8 text-center">
                <button 
                  ...
                  className="font-heading font-semibold text-sm text-electric-blue hover:underline"
                >
                    ...
                    <tr key={session.id} className={index !== 0 ? 'border-t border-outline/20 hover:bg-surface-container-lowest transition-colors' : 'hover:bg-surface-container-lowest transition-colors'}>
                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex justify-end gap-2">
                          <button 
                            onClick={() => router.push(session.status === 'draft' ? `/teacher/session/${session.id}/edit` : `/teacher/session/${session.id}/dashboard`)}
                            className="p-2 text-on-surface-variant hover:text-electric-blue hover:bg-electric-blue/10 rounded-lg transition-colors"
                            title={session.status === 'draft' ? "Edit Kuis" : "Buka Kuis"}
                          >
                            {session.status === 'draft' ? <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34a.995.995 0 00-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg> : <ExternalLink className="w-5 h-5" />}
                          </button>
                          
                          {/* Tombol Nilai (Muncul jika kuis sudah selesai) */}
                          {session.status === 'finished' && (
                            <button 
                              onClick={() => router.push(`/teacher/session/${session.id}/grading`)}
                              className="p-2 text-on-surface-variant hover:text-[#4CAF50] hover:bg-[#4CAF50]/10 rounded-lg transition-colors"
                              title="Lihat Nilai"
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>
                            </button>
                          )}
