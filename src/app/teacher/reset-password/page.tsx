...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        setError(res.error.message || 'Gagal mereset password');
      } else {
        setMessage('Password Anda berhasil direset! Silakan login dengan password baru.');
        setTimeout(() => {
          router.push('/teacher');
        }, 3000);
      }
          ...
          <div className="p-8 sm:p-10">
            {message ? (
              <div className="text-center">
                <div className="bg-[#E8F5E9] text-[#2E7D32] border-2 border-[#4CAF50] p-6 rounded-xl font-bold text-sm mb-6 flex flex-col items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
                  {message}
                </div>
                <p className="text-on-surface-variant text-sm font-sans">Mengarahkan ke halaman login...</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="font-heading font-semibold text-sm text-on-surface-variant px-2 uppercase tracking-wide">Password Baru</label>
                  <input 
                    type="password"
                    value={password}
                ...
                <div className="space-y-2">
                  <label className="font-heading font-semibold text-sm text-on-surface-variant px-2 uppercase tracking-wide">Konfirmasi Password Baru</label>
                  <input 
                    type="password"
                ...
                >
                  ...
                  <div className="relative bg-transparent px-8 py-4 rounded-full flex items-center justify-center gap-2">
                    <span className="font-heading text-xl text-on-primary font-bold tracking-wide">
                      {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin" /> : "Simpan Password Baru"}
                    </span>
                  </div>
                </button>
              </form>
